...
from tracker.api.serializers import TaskEventSerializer, TaskEventSerializerV2
...
class TaskServiceV2(TaskService):
    def __init__(self):
        self.producer = Producer("billing_v2")
        self.billing_service = BillingService()

    @transaction.atomic
    def create_task(self, public_id: str, task_id: str, summary: str, performer_id: str):
        performer = User.objects.get(public_id=performer_id)
        ...
        self.producer.send(
            [v2.TaskPriceUpdated.model_validate(TaskEventSerializerV2(task).data)]
        )
    ...
    def update_performer(
        self, public_id: str, task_id: str, summary: str, performer_id: str
    ):
        task = Task.objects.filter(public_id=public_id).first()
        if not task:
            return self.create_task(public_id, task_id, summary, performer_id)
    ...
    def complete_task(
        ...
        task = Task.objects.filter(public_id=public_id).first()
        if not task:
            task = self.create_task(public_id, task_id, summary, performer_id)
